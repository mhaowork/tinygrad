...
def apply_threading(k:Scheduler) -> Scheduler:
  # This is safe to run even on kernels with BUFFERIZE, since OptOps.THREAD is restricted to globalizable LOOP ranges.
  if not (k.ren.has_threads and k.ren.global_max is not None): return k
  if DEBUG >= 4:
    print(f"THREAD: consider {k.colored_shape()} full_shape={k.full_shape} loop_axes={k.axes_of(AxisType.LOOP)} global_max={k.ren.global_max}")
    print(f"prod(k.full_shape) {prod(k.full_shape)}")
  for threads in [32,16,12,8,6,5,4,3,2]:
    ...
    if threads > k.ren.global_max[0] or resolve(prod(k.full_shape) // (128 << 10) < threads):
      if DEBUG >= 4: print(f"THREAD: skip threads={threads} (too many or too little work)")
      continue
    for axis in k.axes_of(AxisType.LOOP):
      if DEBUG >= 4: print(f"THREAD: try axis={axis} size={k.full_shape[axis]} threads={threads}")
      if k.full_shape[axis] % threads == 0:
        try: k.apply_opt(Opt(OptOps.THREAD, axis, threads))
        except KernelOptError as e:
          if DEBUG >= 4: print(f"THREAD: failed axis={axis} threads={threads}: {e}")
        else:
          if DEBUG >= 4: print(f"THREAD: applied axis={axis} threads={threads}")
        break
