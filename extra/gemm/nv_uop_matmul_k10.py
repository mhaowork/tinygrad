...
# Kernel 10 from the CUDA worklog: warp-tiling SIMT matmul (no WMMA).
# Shapes are tuned for A100-like GPUs, keeping register/shared reuse high.
...
def hand_spec_kernel10():
  ...
  # store A transposed into shared so As[k, :] is contiguous
  As_store = copy(As.reshape(-1, THREADS_PER_BLOCK)[:, tid],
                  a.permute((1, 0)).reshape(-1, THREADS_PER_BLOCK)[:, tid], rng=100)
  ...
  A_col = UOp.placeholder((ITERS_PER_WARP_M, TM), dtypes.float, slot=0, addrspace=AddrSpace.REG)
  A_col = copy(A_col, As[k, :].reshape(WARPS_IN_BLOCK_Y, ITERS_PER_WARP_M, LANES_PER_WARP_Y, TM)[warpIdy, :, laneIdy, :].cast(dtypes.float),
               rng=300, set=True, upcast=True)

  B_row = UOp.placeholder((ITERS_PER_WARP_N, TN), dtypes.float, slot=1, addrspace=AddrSpace.REG)
  B_row = copy(B_row, Bs[k, :].reshape(WARPS_IN_BLOCK_X, ITERS_PER_WARP_N, LANES_PER_WARP_X, TN)[warpIdx, :, laneIdx, :].cast(dtypes.float),
               rng=400, set=True, upcast=True)
  ...
  sink = c_regs[*rngs].store(c_regs.after(k)[*rngs] + A_col[iterWarpM, yt] * B_row[iterWarpN, xt]).end(iterWarpM, iterWarpN, yt, xt)
  ...
  c = c[warpIdy, :, laneIdy, :, warpIdx, :, laneIdx, :]
  sink = copy(c, c_regs.after(sink).cast(dtypes.half), rng=600)
...
if __name__ == "__main__":
  test_matmul(hand_spec_kernel10(), N=N)
